...
Script to process MIMIC-III clinical database.

This script loads raw MIMIC-III data and processes it into a standardized format
...
from src.data_processing.mimic_processor import process_mimic_data
...
def main():
    ...
    try:
        ...
        # Process MIMIC data
        processed_df, report = process_mimic_data()
        
        # Save processing report
        report_path = PROCESSED_DATA_DIR / 'mimic_processing_report.json'
        with open(report_path, 'w') as f:
            json.dump(report, f, indent=2)
        
        # Print summary
        print("\n" + "="*60)
        print("MIMIC-III DATA PROCESSING COMPLETED")
        print("="*60)
        ...
        print(f"\nOutput files:")
        print(f"  Processed data: {PROCESSED_DATA_DIR / 'mimic_processed.csv'}")
        print(f"  Report: {report_path}")
        
        logger.info("MIMIC processing completed successfully")
        return 0
        
    except Exception as e:
        logger.error(f"Error processing MIMIC data: {e}", exc_info=True)
        print(f"\nERROR: {e}")
